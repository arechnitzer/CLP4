...
sub misc_replacements {
  ...
  ## replace eref{CLP317} and change space and : and . to _
  $text =~ (s:\\eref\{CLP317\}\{[^\}]*\}:($x=$&)=~s/[ ,\:,\.]/_/g;$x:ge);
  $text =~ s/\\eref\{CLP317\}\{(.+?)\}/<xref ref=\"\1\"\/>/sg;
  ...
  $text =~ s/\\includegraphics\[(.+?)\]\{(.+?)\}/<image source=\"figs\/\2"\/>/sg;
  $text =~ s/\\includegraphics\{(.+?)\}/<image source=\"figs\/\1"\/>/sg;
...
sub fix_tikz {
  $text =~ s/\\begin\{tikzpicture\}/<\/p><sidebyside>\n<image><latex-image>\\begin{tikzpicture}/g;
  $text =~ s/\\end\{tikzpicture\}/\\end\{tikzpicture\}<\/latex-image><\/image>\n<\/sidebyside><p>/g;
  # now fix any <m> and </m> between CDATA and ENDDATA back into $.

  my @tmp;
  for(split /\n/, $text) {
    if(/<latex-image>/../<\/latex-image>/){
      s/<m>/\$/g;
      s/<\/m>/\$/g;
      # and also fix this section symbol
      s/§/\\S/g;
    }
    push(@tmp, $_);
  }
