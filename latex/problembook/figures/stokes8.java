...
public class stokes8 extends Template {

     static {templateClass = new stokes8() ; }
...
//          fEnv("psFillGray", 0.4) ;
          ...
          fDisk(X, Y, 1.5, 1.5*aspect, FILLED) ;
          fEnv("psFillColor", Black) ;

          fEnv("lineWidth", 1.0) ;
          fCurve(new circ3d(proj,   cx,cy,cz, rx,h2,0, 0,ry,h1), 0,360,OPEN)  ;

          X = (cx+rx)*proj[0][0]+(cy+h2)*proj[0][1]+cz*proj[0][2] ;
          Y = (cx+rx)*proj[1][0]+(cy+h2)*proj[1][1]+cz*proj[1][2] ;
          fEnv("headLength", 6.0*1.4) ;
          fEnv("headHalfWidth", 2.0*1.4) ;
          fArrowhead(X, Y, -22+180) ;
          fTeXlabel(X,Y-4, "tc", "$C$") ;
//          fTeXlabel(X,Y-5, "tc", "$C=\\partial S$") ;
          ...
          fLine3d(cx, cy+dy, cz+dz1,  cx-Xx, cy+dy-Xy, cz+dz1-Xz) ;
          canvas.append( new fPsWrite("[ ] 0 setdash\n")) ;
          fArrow3d(cx-Xx, cy+dy-Xy, cz+dz1-Xz,  cx-XXx, cy+dy-XXy, cz+dz1-XXz) ;
          fTeXlabel3d(cx-XXx, cy+dy-XXy+4, cz+dz1-XXz, "bl", "$\\hn$") ;


          fTeXlabel3d(cx+rx,cy-ry+30,cz-h1-5, "bl", "$S$") ;
