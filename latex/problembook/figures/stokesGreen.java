...
public class stokesGreen extends Template {

     static {templateClass = new stokesGreen() ; }
...
//          dz1 = dy*h1/ry ;
//          X = cx*proj[0][0]+(cy+dy)*proj[0][1]+(cz+dz1)*proj[0][2] ;
//          Y = cx*proj[1][0]+(cy+dy)*proj[1][1]+(cz+dz1)*proj[1][2] ;
          fEnv("psFillGray", 0.8) ;
          fCurve(new circ3d(proj,   cx,cy,cz, rx,h2,0, 0,ry,h1), 0,360,FILLED)  ;
          fEnv("psFillGray", 0.0) ;

          fEnv("psFillColor", Blue) ;
          fEnv("useColorPs", "true") ;
          fDisk3d(cx+dx, cy+dy, cz+dz1, 1.5, 1.5*aspect, FILLED) ;
          fEnv("psFillColor", Black) ;

          fEnv("lineWidth", 1.0) ;
          fCurve(new circ3d(proj,   cx,cy,cz, rx,h2,0, 0,ry,h1), 0,360,OPEN)  ;

          X = cx*proj[0][0]+(cy-ry)*proj[0][1]+cz*proj[0][2] ;
          Y = cx*proj[1][0]+(cy-ry)*proj[1][1]+cz*proj[1][2] ;
          fEnv("headLength", 6.0*1.2) ;
          fEnv("headHalfWidth", 2.0*1.2) ;
          fArrowhead(X, Y, -135) ;
          fTeXlabel(X,Y-1, "tl", "$C$") ;
          ...
          double Xy  = -l*rx*h1 ;
          double Xz  = l*rx*ry ;
          fArrow3d(cx+dx, cy+dy, cz+dz1,  cx+dx+Xx, cy+dy+Xy, cz+dz1+Xz) ;
          fTeXlabel3d(cx+dx+Xx, cy+dy+Xy+1, cz+dz1+Xz, "bl", "$\\hn$") ;


          fTeXlabel3d(cx,cy,cz, "cc", "$R$") ;
