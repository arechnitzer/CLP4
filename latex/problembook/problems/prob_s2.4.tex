...
(b) \textbf{Solution 1:}\\ 
The path is the union of three line segments.
\begin{itemize}\itemsep1pt \parskip0pt \parsep0pt %\itemindent-15pt
\item
...
(b) \textbf{Solution 2:}\\ 
The vector field $\vF$ is conservative. Indeed $\vF=\vnabla\varphi$ with
$\varphi =  \frac{x^2}{2}-zx +\frac{y^2}{2} -zy$. The potential $\varphi$ 
was found by guessing. Alternatively, $\varphi$ can be found 
by antidifferentiating the three components of $\vF$. In any event
\begin{equation*}
\int_\cC\vF\cdot \dee{\vr}=\varphi(1,1,1) -\varphi(0,0,0)
=\left[\frac{1}{2}-1 +\frac{1}{2} -1\right] - 0
=-1
\end{equation*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{question}
Find the mass of a wire formed by the intersection of the
sphere $x^2+y^2+z^2=1$ and the plane $x+y+z=0$ if the density at $(x,y,z)$
is given by $\rho(x,y,z)=x^2$ grams per unit length of wire.
\end{question}

\begin{hint} 
See  \S\eref{CLP317}{sec:paramCircles} in the CLP-4 text for a simple way to parametrize the wire.
\end{hint}
...
\begin{solution} 
The curve in question is the intersection of a sphere with
a plane through the centre of the sphere. So the curve is a circle (in
fact, a great circle on the sphere). A simple way to parametrize such a
circle is to pick two unit vectors $\hi'$, $\hj'$ that are mutually
perpendicular and perpendicular to $(1,1,1)$ (so that they both lie in
the plane $x+y+z=0$). For example $\hi' = \tfrac{1}{\sqrt{2}}(1,-1,0)$
and $\hj'=\tfrac{1}{\sqrt{6}}(1,1,-2)$. (Check the orthogonality requirements
by dotting $\hi'$ and $\hj'$ with each other and with $(1,1,1)$.) Then
\begin{equation*}
\vr(\theta)=\cos\theta\ \hi'+\sin\theta\ \hj'
=\big(\tfrac{\cos\theta}{\sqrt{2}},-\tfrac{\cos\theta}{\sqrt{2}}, 0\big)
+\big(\tfrac{\sin\theta}{\sqrt{6}},\tfrac{\sin\theta}{\sqrt{6}}, -2\tfrac{\sin\theta}{\sqrt{6}}\big)
\end{equation*}
is a parametrization of the circle. As $\hi'$ and $\hj'$ are mutually
perpendicular unit vectors, ${\vr\,}'(\theta)=-\sin\theta\ \hi'+\cos\theta\ \hj'$
is a unit vector and $\tfrac{ds}{d\theta}=1$. So
\begin{align*}
{\rm mass}&=\int_C\rho ds
=\int_0^{2\pi} x(\theta)^2\ \tfrac{ds}{d\theta}\ d\theta
=\int_0^{2\pi}\big[\tfrac{\cos\theta}{\sqrt{2}}
+\tfrac{\sin\theta}{\sqrt{6}}\big]^2\,d\theta \\
&=\int_0^{2\pi}\big[\tfrac{\cos^2\theta}{2}
+2\tfrac{\cos\theta}{\sqrt{2}}
\tfrac{\sin\theta}{\sqrt{6}}
+\tfrac{\sin^2\theta}{6}\big]\,d\theta
\end{align*}
As $\int_0^{2\pi}\cos^2\theta\,d\theta=\int_0^{2\pi}\sin^2\theta\,d\theta
=\half \int_0^{2\pi}[\sin^2\theta+\cos^2\theta]\,d\theta=\pi$
and $\int_0^{2\pi}\cos\theta\sin\theta\,d\theta=\half\int_0^{2\pi}\sin 2\theta\,d\theta=0$,
the mass is $\tfrac{\pi}{2}+\tfrac{\pi}{6}=\tfrac{2}{3}\pi$.
\end{solution}
...
\begin{question}[M317 2001D] %1
	...
	\begin{enumerate}[(a)]
		...
		\item
		Find the curvature $\ka$ at the apex of the flight, which occurs
		at $t=1$.
