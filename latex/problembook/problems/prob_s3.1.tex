...
\begin{question}
This question concerns the parametrization of a solid of rotation about a line that is not a coordinate axis.
...
%\end{hint}
\begin{answer}
(a) $(x,y,z)=\big(2+\frac1{\sqrt 2}\cos\theta~,~2+\frac1{\sqrt 2}\cos\theta~,~4+\sin\theta\big)$, $\ 0 \le \theta \le 2\pi$.

(b) $x=4+(2\sqrt2-\cos\theta)\cos\phi$, $y=4+(2\sqrt2-\cos\theta)\sin\phi$, $z=4+\sin\theta$, $\ 0 \le \theta,\phi \le 2\pi$.
\end{answer}
\begin{solution}
(a) The mutually perpendicular unit vectors $\hk$ and $\frac1{\sqrt 2}(\hi+\hj)$ both lie in the plane $x=y$. So
\begin{align*}
(x,y,z) &= (2,2,4) +\cos\theta\  \frac1{\sqrt 2}(\hi+\hj) + \sin\theta\ \hk \\
      &= \Big(2+\frac1{\sqrt 2}\cos\theta~,~2+\frac1{\sqrt 2}\cos\theta~,~4+\sin\theta\Big)
\end{align*}
with $0 \le \theta \le 2\pi$, is a parametriztion of a circle of radius 1, centred at $(2,2,4)$, that lies in the plane $x=y$. See the sketch below.
...
\fill[opacity=0.2, blue] (0,0)rectangle(6.5,6.5);
\YEycoord{4}{4}
...
(b) Concentrate on a point $(x,y,z)=(2+\frac1{\sqrt 2}\cos\theta~,~2+\frac1{\sqrt 2}\cos\theta~,~4+\sin\theta)$ of the circle. Rotate it $\phi$ radians about the vertical line $x=y=4$.
\begin{center}
\begin{tikzpicture}
...
\end{tikzpicture}
\end{center}
The new position of the point has the same height, $z=4+\sin\theta$, as the original point. Its distance from the line $x=y=4$ is also the same as that of the original point. That distance is 
\begin{align*}
 R&=\sqrt{(x-4)^2+(y-4)^2+(z-z)^2} \\
  &=\sqrt{\Big(\frac1{\sqrt 2}\cos\theta-2\Big)^2+\Big(\frac1{\sqrt 2}\cos\theta-2\Big)^2+0} \\
  &= \sqrt{2\Big(\frac1{\sqrt 2}\cos\theta-2\Big)^2} \\
  &= \sqrt{(\cos\theta-2\sqrt{2})^2} \\
   &=|\cos\theta-2\sqrt2| \\
   &=2\sqrt2 - \cos\theta
 \end{align*}

The circle traced out by a point $(x,y,z)=(2+\frac1{\sqrt2}\cos\theta,2+\frac{1}{\sqrt 2} \cos \theta,4+\sin\theta)$ on the circle of part (a) is centred at $(4,4,z)$ and has radius $2\sqrt2 - \cos\theta$. So it has equation 
 $x=4+(2\sqrt2-\cos\theta)\cos\phi$, $y=4+(2\sqrt2-\cos\theta)\sin\phi$, $z=4+\sin\theta$.
\end{solution}
%%%%%%%%%%%%%%%%%%
