...
public class helix extends Template {

     static {templateClass = new helix() ; }
...
//          double x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          fCurve(new helix3d(al, 0.5,   0,0,0, r*0.97, h),0,180,OPEN)  ;
          canvas.append( new fPsWrite("[3 3] 1.5 setdash\n")) ;
          fCurve(new helix3d(al, 0.5,   0,0,0, r*0.97, h),180,360,OPEN)  ;
          canvas.append( new fPsWrite("[ ] 0 setdash\n")) ;
          fCurve(new helix3d(al, 0.5,   0,0,0, r*0.97, h),360,540,OPEN)  ;
          double xx = 0; double yy = r; double zz = 0;
          double X = xx*x0+yy ;  double Y = xx*x1+zz ;
          double R = 2.5;
          fEnv("psFillColor", Red) ;
          fDisk(X, Y, R, R*aspect, FILLED) ;
          fTeXlabel(X-5,Y-5, "tl", "$t=0$") ;
          xx = 0; yy = -r; zz = h*180/100;
          X = xx*x0+yy ;  Y = xx*x1+zz ;
          fDisk(X, Y, R, R*aspect, FILLED) ;
          fTeXlabel(X-5,Y, "cr", "$t=\\frac{\\pi}{2}$") ;
          xx = 0; yy = r; zz = h*360/100;
          X = xx*x0+yy ;  Y = xx*x1+zz ;
          fDisk(X, Y, R, R*aspect, FILLED) ;
          fTeXlabel(X+5,Y, "cl", "$t=\\pi$") ;
