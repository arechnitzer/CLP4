...
public class curvatureSignB extends Template {

     static {templateClass = new curvatureSignB() ; }

     public void setup() {
          filePrefix = "curvatureSignB" ;
          outputPsFile = true ;
          outputLblFile = true ;
          xsize = 2.2 ;   
          ysize = 2.2 ;   
     ...
     public  void prepareFigure(Figure canvas) {
          ...
          double alDeg = -22.5 ; double al = alDeg*Math.PI/180 ;
          ...
          fEllipticalArc(xc, yc, rho, rho, -90, 0, OPEN) ;

          fEnv("headHalfWidth", 2.0*1.5) ;
          fEnv("headLength", 6.0*1.5) ;
          fArrowhead(xArr, yArr, -112.5) ;

          fArrow(x, y, x+Tx, y+Ty) ;
          fTeXlabel(x+Tx+0.15, y+Ty, "bl", "$\\hat\\vT$") ;
          ...
          fEnv("lineWidth", 0.5) ;
          fLine(x, y, x-Tx, y) ;
          fEllipticalArc(x, y, rho/5, rho/5, 0, 270+th*180/Math.PI, OPEN) ;
          double xPhi = x + 0.3*rho*Math.cos( (3*Math.PI/2 +th)/2 ) ;
          double yPhi = y + 0.3*rho*Math.sin( (3*Math.PI/2 +th)/2 ) ;
          ...
          double xL = rho/2 +0.2 ; double yL = -rho - 0.25 ;
