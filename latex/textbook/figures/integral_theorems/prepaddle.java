...
public class prepaddle extends Template {

     static {templateClass = new prepaddle() ; }
...
//          fTeXlabel3d(xm,-1,-1, "tr", "$x$") ;
          ...
          fEnv("lineWidth", 1.0) ;
          fCurve(new circ3d(proj,   cx,cy,cz, r,0,0, 0,r,0), 0,360,OPEN)  ;
...
//          fLine3d(cx-r+dx, cy, cz, cx+r-dx, cy, cz) ;
          ...
          double th = 60*Math.PI/180.0 ;
          double dth = 3*Math.PI/180.0 ;
          fArrow3d(cx+r*Math.cos(th), cy+r*Math.sin(th), cz,
                   cx+r*Math.cos(th+dth), cy+r*Math.sin(th+dth), cz) ;
          fLine3d(cx, cy, cz, cx+r*Math.cos(th-4*dth), cy+r*Math.sin(th-4*dth), cz) ;
          fTeXlabel3d(cx+r*Math.cos(th)/2.2,cy+r*Math.sin(th)/2.2,cz+1, "bl","$\\veps$");
          th = 20*Math.PI/180.0 ;
          fTeXlabel3d(cx+r*Math.cos(th), cy+r*Math.sin(th), cz-3, "tc", "$C_\\veps$") ;
