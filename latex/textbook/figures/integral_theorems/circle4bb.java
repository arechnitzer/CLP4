...
public class circle4bb extends Template {

     static {templateClass = new circle4bb() ; }
...
//          fCurve(new circ3d(proj, cx,cy,cz, 2*rx,0,0, 0,0,2*ry), 0,360,OPEN)  ;
          ...
          fTeXlabel3d(cx, cy+r/3.0+7, cz+r/3.0-2, "cc", "$C$") ;

          double t1, t2, x1, y1, z1, x2, y2, z2 ;
          t1 = 0.6*Math.PI/4 ;
          t2 = 0.7*Math.PI/4 ;
          x1 = 2*rx*Math.cos(t1) ;
          y1 = s2*ry*Math.sin(t1) ;
          z1 = y1 ;
          x2 = 2*rx*Math.cos(t2) ;
          y2 = s2*ry*Math.sin(t2) ;
          z2 = y2 ;
