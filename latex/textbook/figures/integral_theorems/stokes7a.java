...
public class stokes7a extends Template {

     static {templateClass = new stokes7a() ; }
...
//          fTeXlabel3d(xm,-1,-1, "tr", "$x$") ;
          ...
          fEnv("lineWidth", 1.0) ;
          fEnv("psFillGray", 0.8) ;
          fCurve(new circ3d(proj, 0,0,0, 1,0,0, 0,1,0), 0,360,OPEN+FILLED)  ;
          fTeXlabel3d(0, -0.05, 1, "br", "$S$") ;
          fTeXlabel3d(1, 0, -0.05, "tc", "$S'$") ;

          fCurve(new wierd3d(proj,13) , -1, 1,OPEN)  ;

          double len = 0.3 ;
          fTeXlabel3d(-0.3, 0.3, 2*len, "cc", "$V$") ;

          fArrow3d(-0.3, 0.5, 0,    -0.3, 0.5, len) ;
          fTeXlabel3d(-0.3, 0.55, len, "lc", "$\\hn$") ;

          double th = 13*Math.PI/180 ;
          double cth = Math.cos(th) ;
          double sth = Math.sin(th) ;
          double t = -0.4 ;
          double x = sth*t ;
          double y = cth*t ;
          double z = (1-y*y)*(1-y*y*y)*Math.exp(1.5*y);
          double zp = -2*y*(1-y*y*y)*Math.exp(1.5*y)
                      -(1-y*y)*(3*y*y)*Math.exp(1.5*y)
                      +1.5*(1-y*y)*(1-y*y*y)*Math.exp(1.5*y);
          zp = zp*1.3 ;
          double llen = len/Math.sqrt(1+zp*zp) ;
          fArrow3d(x, y, z,    x, y-zp*llen, z+llen) ;
          fTeXlabel3d(x, y-zp*llen, z+llen-0.05, "rc", "$\\hn$") ;
...
class wierd3d implements S2V {
     ...
     public wierd3d(double[][] proj, double th) {
        this.proj = proj ;
