...
public class circle4d extends Template {

     static {templateClass = new circle4d() ; }
...
////  General orthogonal projection.  
     ...
     public  void prepareFigure(Figure canvas) {
          ...
          fEnv("psFillGray", 0.8) ;
          fCurve(new circ3d(proj, cx,cy,cz, 2*rx,0,0, 0,ry*s2,ry*s2), 0,90,FILLED)  ;
          {
          double[] xlist = {cx, cx+2*rx, cx+rx    ,cx} ;
          double[] ylist = {cy, cy,  cy+ry*s2*0.7 ,cy+ry*s2} ;
          double[] zlist = {cz, cz,  cy+ry*s2*0.7 ,cz+ry*s2} ;
