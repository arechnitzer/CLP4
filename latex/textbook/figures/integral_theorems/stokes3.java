...
public class stokes3 extends Template {

     static {templateClass = new stokes3() ; }
...
//          fEnv("psFillGray", 0.6) ;
          ...
          fEnv("lineWidth", 1.0) ;
          fCurve(new circ3d(proj,   cx,cy,cz, rx,h2,0, 0,ry,h1), 0,360,OPEN)  ;
          double t = -15*Math.PI/180 ; 
            double xt = Math.cos(t)*rx ;
            double yt = Math.cos(t)*h2 + Math.sin(t)*ry ;
            double zt =                  Math.sin(t)*h1 ;
            double dxt = -Math.sin(t)*rx ;
            double dyt = -Math.sin(t)*h2 + Math.cos(t)*ry ;
            double dzt =                    Math.cos(t)*h1 ;
            double Xx  = -dyt*zt +dzt*yt ;
            double Xy  = -dzt*xt +dxt*zt ;
            double Xz  = -dxt*yt +dyt*xt ;

          fEnv("headLength", 6.0*1.2) ;
          fEnv("headHalfWidth", 2.0*1.2) ;
          fEnv("lineWidth", 0.5) ;
          fArrow3d(cx+xt, cy+yt, cz+zt,  cx+xt+dxt, cy+yt+dyt, cz+zt+dzt) ;
          fArrow3d(cx+xt, cy+yt, cz+zt,  cx+xt+0.02*Xx, cy+yt+0.02*Xy, cz+zt+0.02*Xz) ;
          fTeXlabel3d(cx+xt, cy+yt, cz+zt-1.8, "tl", 
                      "$\\vR(t_0)=\\vr(u_0,v_0)$") ;
          fTeXlabel3d(cx+xt+dxt, cy+yt+dyt, cz+zt+dzt, "cl", 
                      "$\\frac{\\partial \\vr}{\\partial u}$") ;
          fTeXlabel3d(cx+xt+0.02*Xx, cy+yt+0.02*Xy, cz+zt+0.02*Xz, "bc", 
                      "$\\frac{\\partial \\vr}{\\partial u}\\times"+
...
//          fEnv("arrowHeadGray", 0.5) ;
          fArrow3d(cx+xt, cy+yt, cz+zt,  cx, cy, cz) ;
//          canvas.append( new fPsWrite("0.0 setgray\n")) ;
//          fEnv("arrowHeadGray", 0.0) ;
          fTeXlabel3d(cx, cy, cz, "bl", 
                      "$\\frac{\\partial \\vr}{\\partial v}$") ;
...
//          t = -30*Math.PI/180 ; 
//          xt = Math.cos(t)*rx ;
//          yt = Math.cos(t)*h2 + Math.sin(t)*ry ;
//          zt =                  Math.sin(t)*h1 ;
          X = (cx+xt)*proj[0][0]+(cy+yt)*proj[0][1]+(cz+zt)*proj[0][2] ;
          Y = (cx+xt)*proj[1][0]+(cy+yt)*proj[1][1]+(cz+zt)*proj[1][2] ;
          fEnv("psFillColor", Red) ;
          fDisk(X, Y, 1.7, 1.7*aspect, FILLED) ;
          ...
          Y = (cx+xt)*proj[1][0]+(cy+yt)*proj[1][1]+(cz+zt)*proj[1][2] ;
          fArrowhead(X, Y, 14) ;
          fTeXlabel(X, Y-2.5, "tr", "$\\partial S$") ;
