...
public class phaseHyperbola3 extends Template {

     static {templateClass = new phaseHyperbola3() ; }

     public void setup() {
          filePrefix = "phaseHyperbola3" ;    
          outputPsFile = true ;
          outputLblFile = true ;
          xsize = 2.0 ;      // in inches
          ysize = 2.0 ;   // in inches. 
     ...
     public  void prepareFigure(Figure canvas) {
          ...
          double C = 2 ; x = 3 ;  y = C/x; double th = Math.atan(-C/(x*x))*180/Math.PI ;
          fCurve(new hyp1(C), 0.01, xmax, OPEN) ;
          fArrowhead(x,y, th) ;


          C = -2 ; x = 3 ;  y = C/x ;  th = Math.atan(-C/(x*x))*180/Math.PI ;
          fCurve(new hyp1(C), 0.01, xmax, OPEN) ;
          fArrowhead(x,y, th) ;

          C = 2 ; x = -3 ;  y = C/x ;  th = 180+Math.atan(-C/(x*x))*180/Math.PI ;
